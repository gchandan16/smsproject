...
class StudentService:
    ...
    def _build_section_label(self, student: Student) -> Optional[str]:
        """Return 'Class 10 - A' from the student's active enrollment."""
    ...
    def list_students(
        ...
        for s in students:
            items.append(StudentListOut(
                ...
                current_section=self._build_section_label(s),
            ))
    ...
    def create_student(self, data: StudentCreate, tenant_id: UUID) -> Student:
        # 1. Check admission number unique
        ...
        self.db.add(student)
        self.db.flush()   # get student.id without full commit
        ...
        # 4. Create enrollment if provided
        if data.enrollment:
            self._enroll_student(student.id, data.enrollment, tenant_id)
    ...
    def update_student(
        ...
        logger.info(f"In service student_id:{student_id} | "f"student_data:{data.dict()} | " f"tenant_id:{tenant_id}") 

        # Get only fields that were explicitly sent in the request
        update_data = data.model_dump(exclude_unset=True)

        clean_data = {}
        for key, value in update_data.items():
            if key == 'address' and value is None:
                continue  # never wipe address with null
    ...
    # ── Guardian management ───────────────────────────────────
    def add_guardian(
        self, student_id: UUID, data, tenant_id: UUID
    ) -> Guardian:
    ...
    def update_guardian(
        self, guardian_id: UUID, data, tenant_id: UUID
    ) -> Guardian:
